...
def unread_group_messages_count(request):
    ...
    total = 0
    memberships = GroupMembership.objects.filter(user=request.user, is_muted=False).only('group_id', 'last_read_at')
    for m in memberships:
        qs = GroupMessage.objects.filter(group_id=m.group_id, deleted_at__isnull=True).exclude(
            sender=request.user
